...
from src.chunking.html_chunker import (
...
class ExtractChunkPipeline:
    ...
    SUPPORTED_MARKDOWN_HIERARCHY_EXTENSIONS = {
        ".pdf",
        ".hwp",
        ".hwpx",
        ".docx",
    ...
    def get_candidate_extensions(self) -> set[str]:
        ...
        if strategy in {"hwp_pdf_markdown", "markdown_hierarchy", "hwp_clause_markdown"}:
            return self.SUPPORTED_MARKDOWN_HIERARCHY_EXTENSIONS

        raise ValueError(
            f"지원하지 않는 chunking.strategy입니다: {strategy}. "
            "사용 가능 값: pdf_page, section, hwp_pdf_markdown"
        )
    ...
    def _create_hwp_pdf_markdown_chunks(
        self,
        row: pd.Series,
        doc_id: str,
        file_path: Path,
        ...
        """
        HWP/PDF를 PDF 기반으로 파싱하고 표/조항 위계를 보존한 마크다운 청킹을 수행합니다.
        """
        chunking_cfg = self.config.get("chunking", {})
        columns = self.extract_cfg.get("columns", {})
        ...
        conversion_dir_cfg = (
            chunking_cfg.get("conversion_dir")
            or chunking_cfg.get("converted_pdf_dir")
        )

        if conversion_dir_cfg:
            conversion_dir = resolve_project_path(
                self.project_root,
                conversion_dir_cfg,
            )
        else:
            conversion_dir = self.paths["cleaned_dir"].parent / "converted_pdf"

        chunks = chunk_hwp_or_pdf_by_markdown_hierarchy(
            doc_id=doc_id,
            file_path=file_path,
            file_name=row.get(file_name_col, ""),
            file_type=row.get(file_type_col, file_path.suffix.lstrip(".")),
            project_name=row.get(project_name_col, ""),
            organization=row.get(organization_col, ""),
            max_chars=chunking_cfg.get("max_chars", 1500),
            overlap_chars=chunking_cfg.get("overlap_chars", 150),
            min_chars=chunking_cfg.get("min_chars", 30),
            parent_child_threshold=chunking_cfg.get(
            ...
            max_parent_context_chars=chunking_cfg.get(
                "max_parent_context_chars",
                1800,
            ),
            store_parent_context_in_metadata=chunking_cfg.get(
                "store_parent_context_in_metadata",
                False,
            ),
            store_child_text=chunking_cfg.get(
                "store_child_text",
                False,
            ),
    ...
    def process_single_row(self, row: pd.Series) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if chunking_strategy in {
                ...
                chunks = self._create_hwp_pdf_markdown_chunks(
                    row=row,
                    doc_id=doc_id,
                    file_path=file_path,
                )
